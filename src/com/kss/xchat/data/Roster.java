...
public class Roster {
	...
	private static final String TABLE_COMPANY= "Profiles";
	...
	public ArrayList<Roster> getProfiles()
	{
		DBHelper dbHelper=new DBHelper(context);
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		ArrayList<Roster> compList = new ArrayList<Roster>();
	    // Select All Query
	    String selectQuery = "SELECT  _id,nickname,name,status," +
	    		"city,state,zip,country,ProfileImage FROM " + TABLE_COMPANY;
	    Cursor cursor = db.rawQuery(selectQuery, null);

	    // looping through all rows and adding to list
	    if (cursor.moveToFirst()) {
	        do {
	        	Roster comp= new Roster(context);
	            comp.id=cursor.getInt(0);
	            comp.NickName=cursor.getString(1);
	            comp.Name=cursor.getString(2);
	            comp.Status=cursor.getString(3);
	            comp.City=cursor.getString(6);
	            comp.State=cursor.getString(6);
	            comp.Zip=cursor.getString(6);
	            comp.Country=cursor.getString(6);
	            comp.ProfileImage=cursor.getString(6);
	            compList.add(comp);
	...
	public Profiles getProfile(String nickname)
	{
		DBHelper dbHelper=new DBHelper(context);
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		Cursor cursor=db.rawQuery("select _id,nickname,name,gender,dob,city,state,country,zip,profileimage from "+
				TABLE_COMPANY+" where nickname='"+ nickname+"'", null);
	...
	public void deleteRecord(String nickname)
	{
		...
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		 db.delete(TABLE_COMPANY, KEY_NICKNAME + " = ?",
		            new String[] { String.valueOf(nickname) });
		    db.close();
	}
